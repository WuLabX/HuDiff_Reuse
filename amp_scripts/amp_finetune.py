...
from dataset.amp_dataset import AMP_MaskCollater, get_amp_dataset
...
from utils.loss import OasMaskedHeavyCrossEntropyLoss, MaskedAccuracy
from utils.amp_motifs import load_active_motifs, load_hemolytic_motifs, parse_motif_types
from utils.amp_scorers import ACTIVITY_SCORERS, build_scorer_from_args, parse_scorer_names
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain_ckpt", type=str, default="",
                        help="Path to pretrained checkpoint. Defaults to checkpoints/<mode>/pretrain.pt")
    ...
    for it in range(config.finetune.max_iter + 1):
        ...
        if it % config.finetune.valid_step == 0 and it != 0 or it == config.finetune.max_iter:
            ...
            if val_loss < best_val_loss:
                ...
                logger.info(f"Canonical finetune ckpt -> checkpoints/{args.mode}/{args.mode}.pt")
            else:
                logger.info(
