...
class PDBParser_mod(object):
    ...
    def get_structure(self, struct_id, filename, long_factors=False):
        """Return the structure. 
 
        Arguments: 
        o struct_id - string, the id that will be used for the structure 
        o filename - name of the PDB filename OR an open filehandle 
        """
        if self.QUIET:
        ...
        self.header = None
        self.trailer = None
        # Make a StructureBuilder instance (pass struct_id of structure as parameter)
        self.structure_builder.init_structure(struct_id)

        with as_handle(filename) as handle:
            self._parse(handle.readlines(), long_factors=long_factors)
    ...
    def _parse(self, header_coords_trailer, long_factors=False):
        "Parse the PDB file."
        ...
        self.trailer = self._parse_coordinates(
            coords_trailer, long_factors=long_factors
        )
    ...
    def _parse_coordinates(self, coords_trailer, long_factors=False):
        ...
        for i in range(0, len(coords_trailer)):
            ...
            if record_type == "ATOM  " or record_type == "HETATM":
                ...
                # occupancy & B factor
                if long_factors:
                    occ_and_bfac = line[54:72].split()
                    if len(occ_and_bfac) >= 2:
                        try:
                            occupancy = float(occ_and_bfac[0])
                        except:
                        ...
                        self._handle_PDB_exception(
                            "cant read occupancy and bfactor with long_factors set to True",
                            global_line_counter,
                        )
                        occupancy = 0.0
                        bfactor = 0.0
                    ...
                    try:
                        ...
                        bfactor = (
                            0.0  # The PDB use a default of zero if the data is missing
                        )
                segid = line[72:76]
...
if __name__ == "__main__":
    ...
    p = PDBParser_mod(PERMISSIVE=True)
