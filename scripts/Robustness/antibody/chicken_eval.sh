...
OUT_ROOT="${ROOT_DIR}/results/Robustness/Ab/chicken"
PYTHON_BIN="${PYTHON_BIN:-/mnt/wucy/miniconda3/envs/Hudiff/bin/python}"

export PYTHONPATH="${ROOT_DIR}:${PYTHONPATH:-}"
export T20_PAIR_CACHE_FPATH="${OUT_ROOT}/t20_pair_cache.csv"
...
for temp in 0.6 1.0 1.4; do
  for seed in 2024 2025 2026; do
    sample_path="${OUT_ROOT}/chicken_FR_Temp${temp}/Seed${seed}_FR_Temp${temp}/sample_humanization_result.csv"
    if [[ ! -f "${sample_path}" ]]; then
      echo "Missing sample file: ${sample_path}" >&2
      exit 1
    fi
    echo "Evaluating ${sample_path}"
    "${PYTHON_BIN}" scripts/Robustness/antibody/chicken_eval.py "${sample_path}"
  done
done
