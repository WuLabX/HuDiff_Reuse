...
for temp in 0.6 1.0 1.4; do
  for seed in 2024 2025 2026; do
    sample_path="results/Robustness/Nb/${DATASET}/${DATASET}_${SAMPLE_METHOD}_Temp${temp}/Seed${seed}_${SAMPLE_METHOD}_Temp${temp}/sample_humanization_result.csv"
    if [[ ! -f "${sample_path}" ]]; then
      echo "Missing sample file: ${sample_path}" >&2
      exit 1
    fi
    "${PYTHON_BIN}" scripts/Robustness/nanobody/eval.py "${sample_path}"
  done
done
